...
def export_selected_objects_for_total_calculation(modeladmin, request, queryset):
	selected = request.POST.getlist(admin.ACTION_CHECKBOX_NAME)
	ct = ContentType.objects.get_for_model(queryset.model)
	return HttpResponseRedirect("/export/?ct=%s&ids=%s" % (ct.pk, ",".join(selected)))
export_selected_objects_for_total_calculation.short_description = "View the total cost of the selected orders"
...
class OrderAdmin(admin.ModelAdmin):
	actions = [export_selected_objects_for_total_calculation]
